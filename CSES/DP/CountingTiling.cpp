...
unordered_map<int, vector<int>> prevTilings;
...
void solution() {
	...
	for(int prev = 0; prev < limit; prev++) {
		generateNextTilings(0, n, prev, 0);
	}

	// build the dp tale => how many ways to get to this filing column
	vector<int> prev(limit);
	prev[0] = 1;
	for(int row = 0; row < m; row++) {
		vector<int> curr(limit);
		for(int tile = 0; tile < limit; tile++) {
			for(int prevTile: prevTilings[tile]) {
				curr[tile] += prev[prevTile];
				curr[tile] %= MOD;
			}
		}
		prev = curr;
